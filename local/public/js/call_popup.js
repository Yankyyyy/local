class CallPopup {
	constructor(call_log) {
		console.log("Custom CallPopup instance created");
	...
	setup_call_details() {
		...
		this.call_details = new frappe.ui.FieldGroup({
			fields: [
				...
				{
					fieldtype: "Button",
					label: __("Create New Water Connection"),
					click: this.create_new_customer.bind(this),
					depends_on: () => !this.get_caller_name(),
				},
				{
					fieldtype: "Button",
					label: __("Create New Issue"),
					click: () => frappe.new_doc("Issue", { custom_raised_by_phone_call: this.caller_number }),
					depends_on: () => !this.get_caller_name(),
				},
				{
...
$(document).on("app_ready", function () {
	console.log("Custom CallPopup script loaded");

	// Remove existing popup reference just in case
	erpnext.call_popup = null;

	// Delay binding to override ERPNext's default
	setTimeout(() => {
		frappe.realtime.on("show_call_popup", (call_log) => {
			console.log("Triggering CallPopup");
